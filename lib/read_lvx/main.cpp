...
int main(int argc, char **argv) {
    ...
    const std::string input_file(argv[1]);   // Input file
    const std::string output_file(argv[2]);  // Output file
    std::cout << "Loading file " << input_file << std::endl;

    // Open lvx file with handler
    ...
    // Buffer to store cloud point data retrieved from lvx file
    ...
    std::ofstream txt(output_file.c_str());  // Create output file stream

    int points = 0;
    int r = 0;
    while (r == livox_ros::kLvxFileOk) {
        const auto progress = lvx_file.GetLvxFileReadProgress();  // Get file read progress
        std::cout << progress << std::endl;                       // Cout read progress
        ...
        if (!r) {
            ...
            while (data_offset < data_size) {
                ...
                if (lvx_file.GetFileVersion()) {
                    livox_ros::LvxFilePacket *detail_packet = (livox_ros::LvxFilePacket *)&packet_base[data_offset];
                    eth_packet = (LivoxEthPacket *)(&detail_packet->version);
                    handle = detail_packet->device_index;
                } else {
                    livox_ros::LvxFilePacketV0 *detail_packet = (livox_ros::LvxFilePacketV0 *)&packet_base[data_offset];
                    eth_packet = (LivoxEthPacket *)(&detail_packet->version);
                    handle = detail_packet->device_index;
                }
                ...
                if (data_type == kExtendCartesian) {
                    ...
                    for (int i = 0; i < points_in_packet; i++) {
                        ...
                        txt << point->x << " " << point->y << " " << point->z << " " << (int)point->reflectivity
                            << std::endl;
                    }
                }
